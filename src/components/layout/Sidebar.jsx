...
function AupreaLogo({ collapsed, isMobile, onClose }) {
  return (
    <div className={`flex items-center gap-3 px-4 py-5 border-b border-white/10 ${collapsed ? 'justify-center' : 'justify-between'}`}>
      ...
      {isMobile && !collapsed && (
        <button
          onClick={onClose}
          className="p-2 rounded-lg hover:bg-white/10 text-gray-dark hover:text-white transition-colors"
        >
          <X className="w-5 h-5" />
        </button>
      )}
...
export default function Sidebar({ unanalysedCount = 0, collapsed = false, onCollapsedChange, isMobile = false, onClose }) {
  ...
  return (
    ...
    >
      {/* Logo */}
      <AupreaLogo collapsed={collapsed} isMobile={isMobile} onClose={onClose} />
